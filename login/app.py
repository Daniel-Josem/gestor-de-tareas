...
def profesor():
    ...
    estado = request.args.get('estado')  # puede ser 'pendiente' o 'completada'
    curso_filtro = request.args.get('curso')

    conn = get_db_connection()
    # Obtener todos los cursos con estudiantes activos
    ...
    query = 'SELECT * FROM tareas WHERE id_usuario_asignado = ?'
    ...
    return render_template('profesor.html', cursos=cursos, tareas=tareas, estado=estado, curso_filtro=curso_filtro)
...
def crear_tarea_profesor():
    ...
    conn = get_db_connection()
    conn.execute('''
        INSERT INTO tareas (titulo, descripcion, fecha_vencimiento, prioridad, estado, curso_destino, ruta_archivo, id_usuario_asignado)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    ''', (titulo, descripcion, fecha_vencimiento, prioridad, estado, curso_destino, ruta_archivo, current_user.id))

    # -- NUEVO: Crear notificaciones para estudiantes del curso destino --
